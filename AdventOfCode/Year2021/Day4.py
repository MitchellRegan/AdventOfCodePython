    ...
    for c in range(0, len(calls)):
        ...
        for board in range(0, len(bingoCards)):
            if board not in winningBoards:
                # Checking if the current board has bingo
                if checkBoard(bingoCards[board], calls[:c+1]):
                    # When we have a bingo, we eliminate that board unless it's the last one
                    if len(bingoCards) - len(winningBoards) > 1:
                        winningBoards.append(board)
                    # If this is the last card to win, we find its score
                    else:
                        prevCalls = calls[:c+1]
                        sum = 0
                        for row in bingoCards[board]:
                        ...
                        print("Year 2021, Day 4 solution part 2:", sum * calls[c])
                        return
